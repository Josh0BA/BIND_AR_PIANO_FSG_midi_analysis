...
# df['played_pitches'] = df['detected_states'].apply(lambda states: [[n['pitch'] for n in d['notes']] for d in states])
df['played_notes_pitches'] = df['played_notes'].apply(lambda notes: [n['pitch'] for n in notes])
print(df)

# Export Test and sequence_length to CSV
df[['Test', 'sequence_length']].to_csv('test_sequence_lengths.csv', index=False)
# Export Participant_ID', 'Test','played_notes_pitches to CSV
df[['Participant_ID', 'Test','played_notes_pitches']].to_csv('played_notes_pitches.csv', index=False)


#ein df mit ausgewählten informationen
df_only_state_sequence = df[['Participant_ID', 'Test', 'Attempt', 'state_sequence']].copy()
# print(df_only_state_sequence)

#einzelne spalten in Konsole anzeigen
# for p,t,a,seq,pp in df[['Participant_ID', 'Test', 'Attempt', 'state_sequence','played_pitches']].values:   
#     print(pp)
#     print(t)
#     print(a)
#     print(len(seq))
...
#and also remove not expectet states (for example played 1 and then 3 state one has to be removed otherwise a state to much)
